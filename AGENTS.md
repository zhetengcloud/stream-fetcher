...
- **Runtime-agnostic core**: no runtime-specific APIs in core. Runtime adapters
...
- Prefer npm packages; avoid JSR-specific modules and Deno-only dependencies.
...
- Extract user-facing strings into `src/messages.ts` and export via the
  package's `package.json` subpath.
...
- Mirror reference implementations (e.g., biliup) closely for
  anti-crack/anti-code logic, but adapt to TypeScript idioms.
...
- Mock external HTTP services with `Bun.serve({ port: 0, fetch: ... })`.
...
- Run `bun run lint` before committing to catch style and correctness issues.
- Run `bun run fmt` before committing to ensure consistent formatting.
