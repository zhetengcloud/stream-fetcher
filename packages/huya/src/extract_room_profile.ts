...
export function extractRoomProfile(page: string): Effect.Effect<RoomProfile | null, Error, never> {
  return Effect.gen(function* () {
    ...
    if (!first || typeof first !== "object") {
      return yield* Effect.fail(new Error(messages.errors.streamDataEmpty));
    }
...
function extractJsonAfter(
  ...
  return Effect.gen(function* () {
    ...
    if (!match) {
      return yield* Effect.fail(new Error(messages.errors.roomDataNotFound));
    }
    const start = match.index + match[0].length;
    const endIdx = page.indexOf(end, start);
    if (endIdx === -1) {
      return yield* Effect.fail(new Error(messages.errors.roomDataIncomplete));
    }
...
function extractStreamJson(page: string): Effect.Effect<Record<string, unknown>, Error, never> {
  return Effect.gen(function* () {
    ...
    if (start === -1) {
      return yield* Effect.fail(new Error(messages.errors.streamDataNotFound));
    }
