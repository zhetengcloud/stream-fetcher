...
function intervalStream(
  ...
  return Stream.fromIterable(chunks).pipe(Stream.tap(() => Effect.sleep(intervalMs)));
}
...
test("record copies source bytes to a single file sink", async () => {
  const { fs, files } = createInMemoryFs();
  const chunks = [new TextEncoder().encode("hello "), new TextEncoder().encode("world")];

  const progressStream = record(byteStream(chunks), new FileSink(), { path: "/tmp/test.bin", fs });

  await Effect.runPromise(collectProgress(progressStream));

  expect(new TextDecoder().decode(files.get("/tmp/test.bin"))).toBe("hello world");
});

test("record emits progress metrics and completes", async () => {
  ...
  const progressStream = record(intervalStream(chunks, 100), new FileSink(), {
    path: "/tmp/progress.bin",
    fs,
  });

  const progressEvents = await Effect.runPromise(collectProgress(progressStream));

  expect(progressEvents.length).toBeGreaterThanOrEqual(1);
  expect(progressEvents[0].bytes).toBe(0);
  expect(progressEvents[0].chunkCount).toBe(0);
  expect(progressEvents[progressEvents.length - 1].bytes).toBeGreaterThan(0);
  expect(progressEvents[progressEvents.length - 1].chunkCount).toBeGreaterThan(0);
});

test("record stops early when aborted", async () => {
  const { fs, files } = createInMemoryFs();
...
test("record stops early on stream cancellation", async () => {
  const { fs } = createInMemoryFs();
  let chunkCount = 0;
  ...
  const progressStream = record(source, new FileSink(), { path: "/tmp/cancelled.bin", fs });

  const stream = await Effect.runPromise(Stream.toReadableStreamEffect(progressStream));
