...
test("HlsSource emits concatenated segment bytes for VOD playlist", async () => {
  ...
  try {
    ...
    const total = new TextDecoder().decode(await Effect.runPromise(readAll(stream)));
...
test("HlsSource refreshes live playlist and fetches new segments", async () => {
  const encoder = new TextEncoder();
  ...
  try {
    ...
    expect(total).toBe("seg0seg1seg2seg3seg4");
  } finally {
    await server.stop();
  }
});

test("HlsSource supports relative and absolute segment URLs", async () => {
  const encoder = new TextEncoder();
  ...
  try {
    ...
    expect(total).toBe("absoluteremote");
  } finally {
    await server.stop();
  }
});
